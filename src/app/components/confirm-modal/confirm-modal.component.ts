      ...
      <div class="relative bg-white rounded-lg shadow-xl w-full max-w-sm p-5 animate-scale-in">
        ...
        @if (showInput()) {
          <input
            ...
            (keyup.enter)="confirm.emit(inputValueModel)"
            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none focus:border-green-600 focus:ring-1 focus:ring-green-200 mb-4"
        ...
        <div class="flex gap-2 justify-end">
          ...
          <button 
            (click)="showInput() ? confirm.emit(inputValueModel) : confirm.emit()"
...
export class ConfirmModalComponent implements OnInit {
  ...
  confirm = output<string | void>();
  cancel = output<void>();
  
  inputValueModel = '';
  
  ngOnInit(): void {
    this.inputValueModel = this.inputValue();
  }
}
